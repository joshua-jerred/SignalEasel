...
PskMode GetPskMode(const mwav::data::Mode modulate_type) {
  switch (modulate_type) {
  case mwav::data::Mode::BPSK_125:
  case mwav::data::Mode::BPSK_250:
  case mwav::data::Mode::BPSK_500:
  case mwav::data::Mode::BPSK_1000:
    return PskMode::BPSK;
  case mwav::data::Mode::QPSK_125:
  case mwav::data::Mode::QPSK_250:
  case mwav::data::Mode::QPSK_500:
    return PskMode::QPSK;
  default:
...
double GetSymbolRate(const mwav::data::Mode modulate_type) {
  switch (modulate_type) {
  case mwav::data::Mode::BPSK_125:
  case mwav::data::Mode::QPSK_125:
    return 125.0;
...
void addPostamble(BitStream &bit_stream, int postamble_length, PskMode mode) {
  ...
  for (int i = 0; i < postamble_length; i++) {
    bit_stream.addBits(((mode == PskMode::QPSK && kFldigiMode) ? zeros : ones),
                       1);
  }
}

void addSymbol(wavgen::Generator &wavgen, PskWave &wave, double shift,
               int filter_end) {
...
void encodeBpsk(wavgen::Generator &wavgen, BitStream &bit_stream,
                PskWave &wave) {
...
void encodeQpsk(wavgen::Generator &wavgen, BitStream &bit_stream,
                PskWave &wave) {
...
bool modulators::pskEncodeAscii(wavgen::Generator &wavgen,
